...
def data_reduction(imageFullname, d_in_pixel, Rot, tilt, lamda, x0, y0, PP):
    ...
    imArray = np.array(im)
    # imArray = imArray[500:2500, 500:2500]
    ...
    cake, Q, chi = p.integrate2d(imArray, 2880, 2880, mask=detector_mask, polarization_factor=PP)
    Q = Q * 10e8
    chi = chi + 90

    Qlist, IntAve = p.integrate1d(imArray, 2880, mask=detector_mask, polarization_factor=PP)
